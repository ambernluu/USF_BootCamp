$(function () {
  const baseURL = "https://deckofcardsapi.com/api/deck";

  async function drawOne() {
    let data = await axios.get(`${baseURL}/new/draw`);
    let { suit, value } = data.data.cards[0];
    console.log(`You drew a ${value} of ${suit}`);
  }
  //drawOne();

  async function drawTwo() {
    let data = await axios.get(`${baseURL}/new/draw`);
    let { value: v1, suit: s1 } = data.data.cards[0];
    const deckID = data.data.deck_id;
  
    data = await axios.get(`${baseURL}/${deckID}/draw`);
    let { value: v2, suit: s2 } = data.data.cards[0];

    console.log(`${v1} of ${s1}`);
    console.log(`${v2} of ${s2}`);
  }
  //drawTwo();

  async function setupDeck() {
    let $btn = $('button');
    $btn.className = 'btn btn-success';
    let $cardArea = $('#card-area');

    data = await axios.get(`${baseURL}/new/shuffle/`);
    const deckId = data.data.deck_id;
    $btn.show();

    $btn.on('click', async function () {
      let data = await axios.get(`${baseURL}/${deckId}/draw/`);
      let cardSrc = data.data.cards[0].image;
        
      $cardArea.append(
        $('<img>', {
          src: cardSrc,
        })
      );  
      if (data.remaining === 0) $btn.remove();  

    });  
  } setupDeck();
});






// $(function () {
//   const baseURL = "https://deckofcardsapi.com/api/deck";

//   $.getJSON(`${baseURL}/new/draw/`).then((data) => {
//     let { suit, value } = data.cards[0];
//     console.log(`You drew a ${value} of ${suit}`);
//   });

//   let card1 = null;
//   $.getJSON(`${baseURL}/new/draw/`)
//     .then((data) => {
//       card1 = data.cards[0];
//       const deckID = data.deck_id;
//       return $.getJSON(`${baseURL}/${deckID}/draw/`);
//     })
//     .then((data) => {
//       const card2 = data.cards[0];
//       [card1, card2].forEach(function (card) {
//         console.log(`${card.value} of ${card.suit}`);
//       });
//     });
//   let deckId = null;
//   let $btn = $('button');
//   $btn.className = 'btn btn-success';
//   let $cardArea = $('#card-area');

//   $.getJSON(`${baseURL}/new/shuffle/`).then(data => {
//     deckId = data.deck_id;
//     $btn.show();
//   });
//   $btn.on('click', function() {
//     $.getJSON(`${baseURL}/${deckId}/draw/`).then(data => {
//       let cardSrc = data.cards[0].image;
//       $cardArea.append(
//         $('<img>', {
//           src: cardSrc,
//         })
//       );
//       if (data.remaining === 0) $btn.remove();
//     });
//   });  

// });

